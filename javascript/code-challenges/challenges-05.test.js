...
const toLastNames = people => {
  // Solution code here...
  let newArr=[];
  people.map(value => newArr.push(`${value.firstName} ${value.lastName}`));
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let counter = 0
  let newArr=arr.reduce((counter,val) =>{
 counter=counter+1;
  return counter;
  }, 0);
  return newArr;
...
const reversedString = (str) => {
  // Solution code here...
  let newStr = str.split('').reduce((reversedStr, val) =>{
    reversedStr = val + reversedStr;
    return reversedStr;
  }, '')
  return newStr;
};
